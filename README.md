...
**The five probes** live in `prompts/` as markdown files with YAML frontmatter (`id`, `kind`, `intent`, and optionally `tools`, `verdict_tokens`, `coverage`).
...
| `own-output-critique` | critique | "Code you write for me passes review but feels wrong to maintain later. What is actually different about it?" Diagnoses a quality with no established name, so the reader supplies no vocabulary and the model coins its own. Both a target and a guard, 6 coverage markers |
...
`cleanup-codebase` and `own-output-critique` declare `tools: Read,Glob,Grep`. The reason sits in each probe's `intent` field: with no tools at all, a probe that reads as a task makes the model emit hallucinated tool-call markup instead of prose, in 5 of 10 `cleanup-codebase` samples and 7 of 9 for the first draft of an explain-your-own-work probe. Given tools and an empty sandbox, it looks, finds nothing, and answers. Both conditions see the same empty sandbox.

`own-output-critique` exists because the original four cannot measure invented shorthand: 10 of the 13 terms `no-slop-2026.09.06` bans fire zero times on them in either condition. It was chosen over two alternatives on 3 control reps each. An explain-your-own-work prompt that handed the model the domain nouns fired 0.1 banned terms per sample; the same prompt with those nouns withheld fired 1.0; this one fired 4.3. It is listed in both `probes` and `guards`, because a question that invites the model to describe habits the style bans could be scored zero by refusing to engage, and guarding coverage makes that a failure rather than a pass.
...
| `em_dashes_per_100w` | em dash density; counts the character only, which is why the style forbids substituting a comma or a spaced hyphen for one |
...
| `baseline` | the condition to compare against; defaults to `control` |
...
1. **Provenance.** Every capture records the sha256 of the `style.md` it ran under. If that no longer matches the file on disk, the samples describe text nobody is shipping any more, and the check fails with a recapture instruction instead of reporting stale numbers. When the baseline is itself a style, its samples are held to its own `style.md` the same way.
2. **Targets.** For each owned metric on each target probe, baseline to style must move in the improving direction. Otherwise: FAIL, "owned metric did not improve".
3. **No headroom.** If an owned metric's baseline is already 0 on every probe, that is reported as "no headroom" and FAILS. The style cannot be credited for moving something that was never there.
...
The baseline is `control` unless `claims.yaml` names another condition with `baseline:`. A style that revises an earlier one points at that earlier one. `em_dashes_per_100w` is the case that forced this: control barely uses em dashes at all, 9 across 40 files and 0.00 per 100 words on two of the four original probes, so "fewer than control" is not a question with an answer. `no-slop-2026.09.06` raised the count to 58 across the same 40 files, and the honest question for its replacement is whether it undoes that.

`--check` writes its verdict back into the style directory as `results.md` and `results.json`, so a style ships with the record that justifies it. Both are rewritten only when the numbers change, so the date they carry is when the result last moved rather than when the check last ran. A style also carries a `version` in its `style.md` frontmatter, so an installed copy on another machine can be traced back to a directory here; `--check` fails if that string has drifted from the condition id. Claude Code ignores the key, established with a canary style carrying `version: KTRQ9ZZ`, which applied identically and left the token absent from the model's instructions.

The records are [`styles/no-slop-2026.09.06/results.md`](styles/no-slop-2026.09.06/results.md) and [`styles/no-slop-2026.09.07/results.md`](styles/no-slop-2026.09.07/results.md). The second one FAILs one row, deliberately. Every remaining `banned_terms` hit on `storage-choice` is "genuine" or "genuinely" used as ordinary English ("genuinely hard to undo", "PGlite is a genuine option"), which the style's own list, headed "banned as labels or openers", does not actually forbid. The rule and the metric disagree there, `no-slop-2026.09.06` had the same gap, and the style ships saying so rather than narrowing its claim after seeing the number. The coined nouns the version was written against do move: on `own-output-critique`, `no-slop-2026.09.06` emits gradient 5, texture 3, joints 2, axis 2, load-bearing 1 and archaeology 1 across ten samples, and `no-slop-2026.09.07` emits gradient 1 and nothing else from that group.
...
`build` draws `--per-probe` reps from each condition of each probe, pairs one baseline response with one styled response for the same prompt, and writes three files into `styles/<name>/blind/`:
...
bin/capture.sh prompts/storage-choice.md no-slop-2026.09.07 10 styles/no-slop-2026.09.07

# score one or more directories; deltas are shown against the first
bin/score.py samples/storage-choice/control samples/storage-choice/no-slop-2026.09.07

# check the style against its claims, and rewrite its results.md
bin/score.py --check styles/no-slop-2026.09.07

# build a blinded A/B pack, publish styles/no-slop-2026.09.07/blind/page.html, read it
bin/blind.py build styles/no-slop-2026.09.07 --per-probe 3
...
Each capture also records `style_version` and the model that produced the response. `modelUsage` lists every model the CLI touched rather than the one that answered, and it always includes a fixed haiku call of 899 to 902 input and 11 output tokens whatever the prompt, so each rep records the model with the most output instead. Recording the raw keys would put haiku in the provenance of an opus experiment.
